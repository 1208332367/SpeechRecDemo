...
audio_result = ''
my_appid = 'your appid'   # 讯飞应用——语音转写服务 appid
my_secret_key = 'your secret_key'  # 讯飞应用——语音转写服务 secret_key
my_test_audio_path = 'your test_audio_path' # 本地测试音频路径


def return_audio_result(wav_direction):
    ...
    class RequestApi(object):
        ...
        def gene_request(self, apiname, data, files=None, headers=None):
            ...
            response = requests.post(lfasr_host + apiname, data=data, files=files, headers=headers)
            # 关闭持久连接，防止max retries exceeded错误
            s = requests.session()
            s.keep_alive = False
            result = json.loads(response.text)
            if result["ok"] == 0 and apiname != api_get_result:
                # print("{} success:".format(apiname) + str(result))
                return result
            elif result["ok"] == 0 and apiname == api_get_result:
                data = str(result)
        ...
        def upload_request(self, taskid, upload_file_path):
            ...
            try:
                ...
                #print('file index:' + str(file_object.tell()))
                file_object.close()
            return True
    ...
    #upload_file_path = filesort[j]
    #print(upload_file_path)
    
    api = RequestApi(appid=my_appid, secret_key=my_secret_key, upload_file_path=wav_direction)
    api.all_api_request()
    return audio_result


if __name__ == '__main__':
    res = return_audio_result(my_test_audio_path)
    if res == '':
